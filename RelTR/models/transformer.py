...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        # print("Number of elements:", memory.numel())  # Total number of elements
        # print("Dimensions:", memory.dim())  # Number of dimensions
        # print("Requires Grad:", memory.requires_grad)  # Whether it tracks gradients
        # print("Mean:", memory.mean().item())
        # print("Standard Deviation:", memory.std().item())
        # print("Min:", memory.min().item())
        # print("Max:", memory.max().item())


        # memcopy = memory.clone()
        # memcopy = memcopy.squeeze(1)
        # # Reduce tensor to 2D using PCA
        # pca = PCA(n_components=3)

        # tensor_pca = pca.fit_transform(memcopy.detach().numpy())

        # # 3D Scatter Plot
        # fig = plt.figure(figsize=(10, 7))
        # ax = fig.add_subplot(111, projection='3d')

        # # Scatter plot with color mapping (optional: color by sequence index)
        # scatter = ax.scatter(
        #     tensor_pca[:, 0], tensor_pca[:, 1], tensor_pca[:, 2],
        #     c=np.arange(len(tensor_pca)), cmap='viridis', alpha=0.8
        # )
        ...
        # ax.set_title("3D PCA Visualization of Transformer Output")
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        """
            MERGE STUFF
                ...
                WEIGHTS
                    self.self_attn_entity 
                    self.dropout2_entity
                    self.norm2_entity
                    self.cross_attn_entity
                    self.dropout1_entity
                    self.norm1_entity
                    self.forward_ffn_entity
                AT THIS STAGE EVERYTHING SHOULD BE ALREADY ABSTRACT
                    VISUALIZE
                        POSITIONAL EMBEDDINGS WITH HEATMAP
                        


            STUFF TRAINED ONLY ON SIMULATED DATA
            tgt_triplet
            triplet_pos
            so_pos
