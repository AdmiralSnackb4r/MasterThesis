...
def main(args):
    ...
    dataset_train = build_custom_dataset("/mnt/s/Datasets/CityScapes/leftImg8bit", "./datasets/train_dataset.json", image_set='train')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        # test
        torch.save(model.state_dict(), './ckpt/test.pth')
        if args.output_dir:
        ...
        # TODO UNCOMMENT THIS ON CLUSTER
        #test_stats, coco_evaluator = evaluate(model, criterion, postprocessors, data_loader_val, base_ds, device, args)

        writer.add_scalar('loss/train', train_stats['loss'], epoch)
        writer.add_scalar('class_error/train', train_stats['class_error'], epoch)
        ...
        writer.add_scalar('lr/train', train_stats['lr'], epoch)

        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     #TODO SAME HERE **{f'test_{k}': v for k, v in test_stats.items()},
                     'epoch': epoch,
